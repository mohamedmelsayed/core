...
class DepositController extends Controller {
    ...
    protected function depositData($scope = null, $summery = false) {
        if ($scope) {
            $deposits = Deposit::$scope()->with(['user', 'gateway', 'subscription']);
        } else {
            $deposits = Deposit::with(['user', 'gateway', 'subscription']);
        }

        $deposits = $deposits->searchable(['trx', 'user:username', 'subscription:type'])->dateFilter();
